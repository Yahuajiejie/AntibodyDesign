#!/usr/bin/env python3
"""Build an offline antigen cold-start split (unseen antigen, known antibody).

Implements entity_cold_start_protocols.md section 5.4 as a standalone,
G00-callable step. Records, the entity annotation table, and (optionally) a
representation annotation table are loaded separately; annotations are validated
and joined only transiently. Outputs keep the base records schema unless
``--debug-keep-entity-columns`` is given.

Example:
    python scripts/data/build_antigen_cold_start_split.py \\
        --records processed/binding/all_records.parquet \\
        --entity-annotations processed/binding/annotations/entity_annotations.parquet \\
        --output-dir processed/binding/splits/antigen_cold_start \\
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional representation annotation table (enables effective-input audit)",
    )
    ...
    print("protocol=antigen_cold_start")
    print(f"train rows={len(result.train)}")
    print(f"valid rows={len(result.valid)}")
    print(f"test rows={len(result.test)}")
    print(f"protocol-excluded (incl. dual-cold) rows={len(result.excluded_records)}")
