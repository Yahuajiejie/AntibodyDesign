...
def _split_by_group(
    ...
    split_by_group = {
        ...
        **{group_id: "valid" for group_id in valid_groups},
        **{group_id: "test" for group_id in test_groups},
    }
    assigned = records.copy()
    assigned["_assigned_split"] = assigned["group_id"].astype(str).map(split_by_group)
    if assigned["_assigned_split"].isna().any():
        raise RuntimeError("some groups were not assigned to a split")
    return (
        _split_rows(assigned, "train"),
        _split_rows(assigned, "valid"),
        _split_rows(assigned, "test"),
    )


def _split_rows(assigned: pd.DataFrame, split: str) -> pd.DataFrame:
    return (
        assigned.loc[assigned["_assigned_split"] == split]
        .drop(columns=["_assigned_split"])
        .sort_values(["dataset_id", "record_id"], kind="stable")
