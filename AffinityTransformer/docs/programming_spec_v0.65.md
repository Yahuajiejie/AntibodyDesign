...
v0.65 变更：重新定义表示提取、抗原-抗体交互网络和训练目标。v0.6 的标准表、
group、split、防泄露和 pair sampler 规则全部保留。模型侧新增以下强制约定：

1. ESM-2、IgBERT 等预训练模型默认作为冻结的特征提取器，不再默认端到端全量微调。
2. 主实验使用 `frozen_cached`：离线提取 residue/token-level embedding，训练阶段不加载
   基座模型。LoRA 采用独立的 `lora_online` 执行模式，只作为后续消融。
3. 项目的主要可训练容量放在独立的抗原-抗体 interaction Transformer 中。层数配置化，
   首轮比较 4/8/16 层，不预设 16 层一定最好。
4. 抗体 encoder 比较 IgBERT 与 ESM-2；融合方式比较 antibody-only、pooled concat 与
   deep cross-attention；训练目标比较 pointwise、pairwise RankNet 与 listwise ListNet。
5. 三种训练目标共享同一个 scalar scorer。禁止为每个数据集、group、assay 或
   `label_kind` 建独立任务头。
6. `affinity_transformer/model.py` 与 `losses.py` 拆成 `affinity_transformer/model/`
   package；模型、pooling、interaction、head 和 loss 按职责分文件。

## 0. 总体判断

新版 `README.md` 的科学性基本站得住。项目把异构亲和力数据建模为同质 group 内排序学习，而不是全局绝对值回归；这与当前数据特征一致，也与 Spearman 评价指标一致。

数据建模方向没有必须推翻的科学问题，但 v0.6 的“两个在线 encoder + 浅层交互”不再是
v0.65 的推荐训练架构。实现时必须守住以下边界：
...
5. 离线 embedding 必须记录 encoder 名称、不可变 revision、tokenizer revision、选取层、
   sequence hash、dtype 和长序列处理规则；元数据不完整的 cache 不得进入正式实验。
6. token-level embedding 与 pooled embedding 必须区分。deep cross-attention 只能消费
   token-level embedding；pooled concat 才允许只消费 pooled embedding。
7. 缓存 embedding 只消除基座模型前向/反向的训练成本，不会消除 interaction Transformer
   的 attention 激活；仍需报告序列长度分布、截断率、显存和吞吐。
...
4. 离线提取、校验并读取抗体/抗原 embedding。
5. 训练抗原-抗体交互网络和 pointwise/pairwise/listwise 排序目标。
...
AffinityTransformer/
  ...
  scripts/
    ...
    embeddings/
      extract_antibody_embeddings.py # 去重序列并写 antibody embedding cache
      extract_antigen_embeddings.py  # 去重序列并写 antigen embedding cache
      validate_embedding_cache.py    # 校验 manifest、metadata、shard 与 shape
    runs/
      ...
      g05_encoder_ablation.sh        # IgBERT / ESM-2 antibody encoder
      g06_fusion_depth_ablation.sh   # antibody-only / concat / 4-8-16 层 interaction
      g07_objective_ablation.sh      # pointwise / RankNet / ListNet
      g08_encoder_tuning_ablation.sh # frozen cache / LoRA（次级实验）
    prepare/
      binding/
        prepare_all.sh
        merge_records.py
...
5. `model/losses.py`、`metrics.py` 不得合并进 `trainer.py`；`trainer.py` 只负责编排。
...
14. `embeddings/extractors.py` 可以依赖 `transformers`；`model/` 不得 import
    `transformers`，只消费 embedding tensor。
15. `model/__init__.py` 只 re-export 稳定公共 API，不写为了 monkeypatch 测试而改变
    运行时函数绑定的兼容 wrapper。
...
离线 embedding 是标准 records 之后、模型训练之前的独立阶段：

```text
records.parquet
-> 按 sequence_hash 去重抗体/抗原序列
-> 使用固定 encoder + tokenizer revision 前向
-> 保存 residue/token-level embedding shards
-> 写 manifest、metadata 和 QC
-> 训练 dataloader 按 sequence_hash 读取
```

变长 embedding 不写回 `records.parquet`，必须使用分片文件和 manifest：

```text
processed/embeddings/{encoder_id}/{revision_id}/
  manifest.parquet
  metadata.yaml
  qc_summary.csv
...
`manifest.parquet` 至少包含：

```text
sequence_hash
sequence_type              # antibody | antigen
encoder_name
encoder_revision
shard_path
item_key
...
`metadata.yaml` 至少记录 encoder/tokenizer 的精确 revision、选取层、是否移除 special
token、最大长度、长序列策略、dtype、生成代码 commit 和生成时间。正式实验禁止只记录
可能漂移的模型别名。

三种 encoder 执行模式必须明确区分：

1. `frozen_cached`：基座冻结并离线提取；训练时不加载基座。主实验默认。
2. `frozen_online`：基座冻结但在线前向；只用于缓存一致性验证或 smoke test。
3. `lora_online`：基座原权重冻结，只训练 LoRA；必须在线前向，不能复用
   `frozen_cached` 的最终 embedding。

`full_finetune` 不属于 v0.65 默认实验。只有独立 config、显存预算和明确对照目的同时
存在时才允许运行。
...
@dataclass
class EncoderConfig:
    name: str                       # esm2_t12_35M | igbert | ...
    revision: str                   # 正式实验必须固定到不可变 revision
    mode: str                       # frozen_cached | frozen_online | lora_online
    embedding_layer: int            # -1 = 最后一层
    cache_dir: Path | None
    max_length: int | None
    long_sequence_strategy: str     # error | truncate | chunk
...
class InteractionConfig:
    kind: str                       # antibody_only | concat | deep_cross_attention
    d_model: int
    num_layers: int                 # concat/antibody_only 必须为 0
    num_heads: int
    ffn_multiplier: float
    dropout: float
    pooling: str                    # masked_mean | attention_pool
    bidirectional: bool

@dataclass
class ObjectiveConfig:
    name: str                       # pointwise | pairwise_ranknet | listwise_listnet
    temperature: float
    sigma: float
    pointwise_loss: str             # huber | mse
...
8. `deep_cross_attention` 要求 `num_layers >= 1` 且 `d_model % num_heads == 0`。
9. `concat`、`antibody_only` 要求 `num_layers == 0`。
10. `frozen_cached` 要求 cache 存在，且 metadata 与 encoder config 完全匹配。
11. `lora_online` 要求 LoRA 参数完整，并禁止静默读取 frozen embedding cache。
12. objective 与 dataloader 类型不匹配时立即报错，不允许回退到 RankNet。
...
如果默认 PyTorch dataloader 足够，可以不写复杂封装；但只要涉及 padding、mask、pair
或 listwise group batch，就必须集中放在这里。

**v0.65 默认训练接口**：`frozen_cached` 训练不再把 token ids 送进
`AffinityRanker`。dataloader 从 embedding store 读取并输出：

```python
@dataclass
class EmbeddingBatch:
    antibody_embeddings: torch.Tensor       # FloatTensor[B, L_ab, D_ab]
    antibody_mask: torch.Tensor             # BoolTensor[B, L_ab]
    antigen_embeddings: torch.Tensor | None # FloatTensor[B, L_ag, D_ag]
    antigen_mask: torch.Tensor | None        # BoolTensor[B, L_ag]
    labels: torch.Tensor                     # FloatTensor[B]
    record_ids: list[str]
    group_ids: list[str]
...
@dataclass
class ListwiseEmbeddingBatch:
    antibody_embeddings: torch.Tensor       # [G, M, L_ab, D_ab]
    antibody_mask: torch.Tensor              # [G, M, L_ab]
    antigen_embeddings: torch.Tensor | None # [G, M, L_ag, D_ag]
    antigen_mask: torch.Tensor | None        # [G, M, L_ag]
    labels: torch.Tensor                     # [G, M]
    group_member_mask: torch.Tensor          # BoolTensor[G, M]
    record_ids: list[list[str]]
    group_ids: list[str]
```

其中 `G` 是 group batch size，`M` 是该 batch padding 后的最大 member 数。padding
member 不得进入 pooling、softmax 或 loss。`D_ab` 与 `D_ag` 允许不同，由 model
projection 统一到 interaction dimension。

以下 tokenization 接口保留给 embedding extraction、`frozen_online` 与
`lora_online`，不是 `frozen_cached` 主训练路径。在线路径与缓存路径必须通过不同的
collate/build 函数显式选择，禁止根据文件是否存在静默切换。
...
该拼接规则只对明确支持它的 encoder adapter 生效。IgBERT 与 ESM-2 必须分别验证
tokenization、special token 和 heavy/light 表示方式；禁止把 `"VH|VL"` 无条件传给
所有 tokenizer。
...
6. embedding batch 能处理 `D_ab != D_ag`。
7. 相同 sequence hash 在同一 encoder/revision 下只读取同一个 cache item。
8. listwise padding member 的 mask 全 False，且不影响任何有效 member。

### 5.4 `model/` package

负责可训练的 projection、抗原-抗体交互网络、pooling 和 scalar scorer。模型 package
不得加载 HuggingFace 基座模型；它只消费 token-level embedding tensor。

#### 5.4.1 Projection

不同 encoder 的 hidden size 不需要相同：

```text
antibody: [B, L_ab, D_ab] -> LayerNorm -> Linear(D_ab, d_model)
antigen:  [B, L_ag, D_ag] -> LayerNorm -> Linear(D_ag, d_model)
```

两个 projection 都是可训练参数。不得为了迁就旧接口强迫 `D_ab == D_ag`。
...
```text
antibody_only:
  projected antibody tokens -> masked pooling -> scalar scorer

concat:
  projected antibody tokens -> masked pooling --+
                                                   +-> concat -> scalar scorer
  projected antigen tokens  -> masked pooling ----+

deep_cross_attention:
  projected token embeddings -> N x InteractionBlock
  -> antibody/antigen masked pooling -> concat -> scalar scorer
```

`InteractionBlock` 使用 pre-norm、residual、dropout 和 FFN。默认双向交互：

```text
Ab = Ab + Dropout(CrossAttention(LN(Ab), key/value=LN(Ag), antigen_mask))
Ab = Ab + Dropout(FFN(LN(Ab)))
Ag = Ag + Dropout(CrossAttention(LN(Ag), key/value=LN(Ab), antibody_mask))
Ag = Ag + Dropout(FFN(LN(Ag)))
```

`bidirectional=false` 时只运行 antibody-query / antigen-key-value 分支，并使用独立实验名。
`num_layers` 首轮比较 `4/8/16`。16 层是容量上界实验，不是默认答案；最终层数依据固定
validation split 的指标、train-valid gap、吞吐、显存和参数量共同选择。

cross-attention 仍包含 `O(L_ab * L_ag)` 开销。冻结并缓存基座 embedding 不会消除该
矩阵，因此 embedding QC 必须报告长度分布和超长序列处理结果。
...
```text
pair_repr -> LayerNorm -> Linear -> GELU -> Dropout -> Linear(..., 1)
```

输出 `score: FloatTensor[B]`，不经过 sigmoid、softmax 或 clamp。禁止为 dataset、group、
assay 或 `label_kind` 建独立 head。

#### 5.4.4 v0.6 在线模型迁移说明

下面保留的 encoder 注入接口仅描述旧模型和 `frozen_online/lora_online` 适配边界，不再是
`frozen_cached` 目标接口。重构期间可以用它验证数值，但新 `AffinityRanker.forward`
最终应接收 `EmbeddingBatch`，不能在 `model/` 内构造或调用基座 encoder。
...
6. projection 支持 `D_ab != D_ag`。
7. concat 模式不得调用任何 interaction block。
8. 4/8/16 层配置必须准确构造对应数量的 block。
9. 每一层 attention 都必须应用正确方向的 antibody/antigen mask。
10. `frozen_cached` 模型参数中不得出现基座 encoder 参数。

### 5.5 `model/losses.py`

三种“任务头”在代码中定义为三种 objective。它们共享 §5.4 的 scalar scorer，区别只在
batch 组织和 loss；推理 API 不因 objective 改变。

#### 5.5.1 Pointwise baseline

原始 Kd、IC50、fitness 等数值跨 group 不可比，pointwise 不得直接混合回归原值。
训练 target 使用 group 内经验分位 rank，范围 `[0, 1]`；binary 保持 `0/1`。
...
pair 两侧必须共享同一个 `AffinityRanker`，不得复制两个不共享参数的网络。

#### 5.5.3 Listwise ListNet
...
```text
target_prob = softmax(group_normalize(labels) / temperature)
pred_log_prob = log_softmax(scores / temperature)
loss = -sum(target_prob * pred_log_prob)
```

规则：

1. `group_normalize` 必须是组内单调变换；零方差 group 不参与 loss 并计入 QC。
2. 相同 label 产生相同 target logit，不得随机打破 ties 制造伪顺序。
3. padding member 在 target/predicted softmax 前 mask，不得影响 loss。
4. batch loss 默认对有效 group 等权平均，避免大 group 仅因 member 多而支配梯度。
...
`Trainer` 是依赖注入的：`model` 和 DataLoader 在传入前已经构建完成。Trainer 不加载
基座 encoder、不读取 embedding shard，也不关心 embedding 来自 ESM-2、IgBERT 还是
测试用 fake cache；它只根据 `config.model.objective.name` 选择训练 step。

构造参数补充说明：

- `train_dataloader`：根据 objective 分别产出 `EmbeddingBatch`、
  `PairEmbeddingBatch` 或 `ListwiseEmbeddingBatch`。类型不匹配立即报错。
- `valid_dataloader`：始终按 record 产出 `EmbeddingBatch`，每条记录只打一次分；
  为 `None` 时 `fit` 跳过验证。
...
1. trainer 不读取原始 CSV。
2. trainer 不构造 pair/group 规则，只消费 dataset/dataloader 输出。
...
4. 三种 objective 都能在 fake embedding 上完成一个 optimizer step。
5. checkpoint 保存 embedding metadata hash；加载时 hash 不匹配立即失败。
...
`build_ranker` 位于 `model/` 或专用 factory，只构造 projection、interaction 和 scalar
head，不 import `transformers`。`build_embedding_extractor` 位于
`embeddings/extractors.py`，是唯一允许构造 HuggingFace encoder/tokenizer 的位置。

IgBERT 与 ESM-2 adapter 各自定义 tokenization、special-token 去除、hidden layer 选择和
sequence length 规则。adapter 输出统一为 `(embedding[L, D], mask[L])`，但不要求
hidden dimension 相同。`transformers` 必须 lazy import，不得在 package import 时加载
模型或读取 cache。

规则：

1. encoder 名称、revision、mode 和 LoRA 参数校验必须在 import `transformers` 或联网前完成。
2. 未实现 encoder 立即报错，不允许退化到 ESM-2。
3. `frozen_cached` 的 `build_ranker` 不得构造任何基座 encoder。
4. cache metadata 中的 hidden dim 用于构造 projection；不再要求两个 encoder
   hidden size 等于 interaction `d_model`。
5. `user_entry.py` 可以复用 extractor 在线编码用户查询，但训练 cache 读取与用户推理
   仍是两个清晰入口。
...
3. 调用 `splits.py` 生成
   train/valid/test 文件。
4. 读取 train/valid/test 标准表，并按 objective 构造 records、pairs 或 groups。
5. 校验 antibody/antigen embedding manifest 与 config、metadata hash、sequence coverage。
6. 构造 embedding-backed dataloader、`AffinityRanker` 和 `Trainer`。
...
6. cache 缺 record、revision 不一致或 shape 不一致时，在 optimizer 创建前失败。
7. 三种 objective 都能使用同一 valid/test record scorer 输出 group-level 指标。
...
class AffinityPredictor:
    ...
    model: AffinityRanker
    config: Config
    antibody_extractor: EmbeddingExtractor
    antigen_extractor: EmbeddingExtractor | None
...
2. 外部用户不需要手动传入 `AffinityRanker`、extractor、checkpoint path 或 config path。
3. `model_name` 从模型注册表解析为 checkpoint/config/encoder 配置；默认值
   `"best"` 指向当前推荐模型。
...
1. `load_predictor` 负责读取注册表、加载 config/checkpoint、构造 ranker 和 embedding extractor。
2. `load_model` 只作为内部/开发者 API：输入 checkpoint/config，返回裸
   `AffinityRanker`；它不挂 extractor，也不作为比赛用户主入口。
3. `AffinityPredictor` 是推理打包对象，包含 `model + extractor + config`。
   extractor 不进入 `AffinityRanker.state_dict()`；frozen 模式下不参与训练参数。
4. 未知 `model_name` 必须抛 `ValueError`，并列出可用模型名。
5. `score_antibodies_with_predictor` / `rank_antibodies_with_predictor` 负责把
   `AntibodyInput` 和抗原序列编码成 `EmbeddingBatch`，再调用
   `predictor.model.forward`。单次调用内相同抗原只编码一次。
...
embedding cache rejects encoder/revision metadata mismatch
embedding extraction deduplicates identical sequence hashes
cached and frozen-online embeddings match within declared tolerance
embedding batch supports different antibody/antigen hidden dimensions
ranknet_loss rewards correct ordering
pointwise loss rewards scores closer to group-rank targets
listnet loss rewards a score ordering matching group labels
listnet loss ignores padding members and preserves label ties
...
concat mode never calls interaction blocks
deep interaction constructs exactly 4/8/16 configured blocks
every interaction layer respects antibody and antigen masks
...
-> build tiny fake embedding cache -> validate cache metadata
-> build records/pairs/groups for the selected objective
-> create embedding-backed dataloader -> forward
-> selected loss -> one optimizer step -> evaluate
-> save checkpoint/metrics/predictions
```

### 7.4 v0.65 对照实验

所有正式对照使用同一 records 子集、同一 group-holdout split、同一 seed 集合和同一
评价代码。每个主实验至少运行 3 个 seed，并报告均值与标准差。

**G05 抗体 encoder**：固定抗原 ESM-2 frozen cache、fusion、objective 和可训练网络
宽度，只比较 `g05_ab_esm2` 与 `g05_ab_igbert`。同时报告 embedding dim、有效最大
长度、截断率与 cache coverage。

**G06 fusion/depth**：固定 encoder cache 与 RankNet，运行：

```text
g06_antibody_only
g06_concat
g06_cross_attention_l4
g06_cross_attention_l8
g06_cross_attention_l16
```

必须报告 trainable parameter count。若要声称收益来自“交互方式”而不是参数量，增加
parameter-matched 配置。

**G07 objective**：固定最佳 encoder/fusion，比较：

```text
g07_pointwise
g07_pairwise_ranknet
g07_listwise_listnet
```

三者使用完全相同的 train/valid/test records；必须报告每个 epoch 实际消费的 unique
records、pairs/groups 和 optimizer steps。

**G08 encoder update（次级）**：主架构确定后再比较 `g08_frozen_cached` 与
`g08_lora_online`。LoRA 必须记录 target modules、rank、alpha、dropout、可训练参数量
和峰值显存。v0.65 默认不做 full fine-tune。
...
checkpoint.pt
run.log
embedding_metadata_refs.yaml # encoder/revision/metadata hash/cache coverage
resource_metrics.json        # trainable params、峰值显存、吞吐
...
5. checkpoint 默认只保存 projection、interaction、scorer，以及 LoRA 模式下的 adapter；
   不重复保存 frozen base encoder。
6. checkpoint 必须引用 embedding metadata hash；加载时不匹配立即报错。
...
13. 禁止默认全量微调两个基座 encoder。
14. 禁止把 pooled embedding 冒充 token-level embedding 送入 deep attention。
15. 禁止在 encoder/tokenizer revision 或 metadata hash 不匹配时继续读取旧 cache。
16. 禁止对 IgBERT 与 ESM-2 无条件套用同一 heavy/light 拼接和 tokenization 规则。
17. 禁止只因为 16 层更深就指定为主模型；必须通过固定 valid split 与资源指标选择。
18. 禁止为每个子数据集、group、assay 或 label_kind 训练独立任务头。
19. 禁止 pointwise 直接回归混合来源的原始 affinity 数值。
20. 禁止 ListNet 随机拆散相同 label 的 ties。
21. 禁止把 LoRA 模式描述成使用预计算最终 embedding；LoRA 必须在线前向。
...
9. 是否保存了配置、随机种子和输出指标。
10. 是否有任何过度生物学宣称。
11. embedding cache 是否固定 revision、保存 metadata hash 并覆盖全部训练记录。
12. 对照实验是否只改变一个目标因素，并报告可训练参数量、显存与吞吐。

## 11. 当前开发优先级

下一阶段按顺序做这些：

1. 把现有 `model.py`、`losses.py` 纯结构拆入 `affinity_transformer/model/`，先保持
   forward 与 RankNet 行为不变，并同步测试 import。
2. 增加 embedding manifest/schema/store，先用 fake embedding 完成单测。
3. 实现 frozen ESM-2 与 IgBERT extractor，完成小批量缓存一致性、长度和 coverage QC。
4. 把训练 dataloader 切换到 embedding-backed batch，同时保留明确命名的 online 路径。
5. 实现 projection、concat、deep interaction block 和统一 scalar scorer。
6. 实现 pointwise 与 ListNet，保留 RankNet，并让 trainer 按 objective 分发。
7. 更新 checkpoint、user entry 和训练入口，确保外部用户仍只提供抗原、抗体和模型名。
8. 先跑 G05-G07；确定主架构后再实现 LoRA/Accelerate 并运行 G08。

本阶段不做这些：

1. 不新增通用 raw CSV parser。
2. 不为每个子数据集训练单独 task head。
3. 不在 G05-G07 之前把 LoRA、full fine-tune 或 MSA fusion 设成默认路径。
4. 不为了追分把 test set 用于调参。

先让 frozen embedding + concat + RankNet 在固定 `group_holdout_split` 上形成可复现闭环，
再逐项增加 deep interaction、objective 对照和 LoRA。不要同时改 encoder、fusion 和 loss。
