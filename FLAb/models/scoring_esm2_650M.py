...
def score(method, df):
    ...
    return corr_ppl, p_value_ppl, corr_ld, p_value_ld, df
...
def process_file(file_path, method_name):
    ...
    corr_ppl, p_value_ppl, corr_ld, p_value_ld, scored_df = score(method, file_path)
    if corr_ppl is None:
    ...
    # Save per-sequence scores (perplexity for each antibody)
    seq_output_path = os.path.join(output_folder, f'{folder_name}_{csv_base}_perseq.csv')
    scored_df.to_csv(seq_output_path, index=False)
    print(f"Per-sequence scores saved to {seq_output_path}")
